...
router.get('/stats', async (req, res) => {
    ...
    try {
        ...
        const [vehicleCount, driverCount, activeTimeEntries, alertCount, ordersToday, ordersYesterday, revenueTodayResult, revenueYesterdayResult, deliveredOrders, allIncompleteOrders] = await Promise.all([
            index_1.prisma.vehicle.count({ where: { tenantId } }),
            index_1.prisma.driver.count({ where: { tenantId } }),
            index_1.prisma.timeEntry.count({
                where: {
                    driver: { tenantId },
                    status: 'RUNNING'
                }
        ...
        for (let i = 6; i >= 0; i--) {
            ...
            const count = await index_1.prisma.order.count({
                where: {
                    tenantId,
...
router.get('/activities', async (req, res) => {
    ...
    try {
        ...
        const recentTracking = await index_1.prisma.timeEntry.findMany({
            where: {
                driver: { tenantId },
                status: 'RUNNING',
            },
