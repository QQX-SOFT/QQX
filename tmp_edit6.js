...
// 2. Add hooks list inside component header
...
// 4. Update footer buttons section cleanly with exact match
const targetFooter = `{!filter && (
                                <button 
...
if (content.indexOf(targetFooter) !== -1) {
    content = content.replace(targetFooter, replacementFooter);
    console.log("Footer buttons updated cleanly!");
} else {
    // Exact spaces may differ, use regex only matched to contiguous button block 
    const fallbackRegex = /\{\!filter \&\& \(\s*<button[\s\S]*?onClick=\{\(\) => handleAcceptOrder[\s\S]*?Auftrag Annehmen[\s\S]*?<\/button>\s*\)\}/g;
    content = content.replace(fallbackRegex, replacementFooter);
    console.log("Footer buttons updated via resilient contiguous matching!");
}

// 5. Append Signature Modal inside return body layout safely
